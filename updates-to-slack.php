...
// Ensure the event is called whenever WordPress loads
...
// Register the deactivation method
...
function updates2slack_sendUpdateInfoToSlack($test_mode = 0)
{
    ...
    foreach ($updates_for_plugins->response as $plugin) {
        if (! is_array(get_option('updates2slack_ignore_plugins'))
            || is_array(get_option('updates2slack_ignore_plugins')) && ! in_array(strtolower($plugin->slug), get_option('updates2slack_ignore_plugins'))) {
            $updates .= '{
    			"type": "section",
    			"text": {
    				"type": "mrkdwn",
	           ...
	           "blocks": [
                   {
                       ...
                       "text": {
                           "type": "mrkdwn",
                           "text": "There are a number of updates available on *'.$site_name.'*",
                       }
                   },';
        }
 ...
 <div>
  ...
  <td>
	  <select name="updates2slack_enabled">
		  <option value="no"<?php if (strtolower(esc_attr(get_option('updates2slack_enabled'))) == 'no') {
		      echo ' selected';
		  } ?>>No</option>
		  <option value="yes"<?php if (strtolower(esc_attr(get_option('updates2slack_enabled'))) == 'yes') {
		      echo ' selected';
		  } ?>>Yes</option>
	  </select>
  </td>
        ...
        if (is_int($cron_time)) {
            if (1 <= count($data)) {
                foreach ($data as $cron_name => $cron_data) {
                    if ($cron_name == CRON_JOB_REF_UPDATES2SLACK) {
  ...
  <td>
	  <input type="date" name="updates2slack_nextruntime_date" value="<?php if ($already_scheduled) {
	      echo $cron_dt->format('Y-m-d');
	  } // get_option('updates2slack_nextruntime_date');?>" /> at <input type="time" name="updates2slack_nextruntime_time" value="<?php if ($already_scheduled) {
	      echo $cron_dt->format('H:i');
	  } // get_option('updates2slack_nextruntime_time');?>" />
	  <br /><em>Please note that this is your server's time (currently set to <?php $thistime = new DateTime();
    echo $thistime->format('H:i'); ?>) and may not reflect your locality</em>
  </td>
  </tr>
  <tr valign="top">
  ...
  <td>
	  <select name="updates2slack_frequency">
		  <option value="daily"<?php if (esc_attr(strtolower(get_option('updates2slack_frequency'))) == 'daily') {
		      echo ' selected';
		  } ?>>Daily</option>
		  <option value="weekly"<?php if (esc_attr(strtolower(get_option('updates2slack_frequency'))) == 'weekly') {
		      echo ' selected';
		  } ?>>Weekly</option>
		  <option value="monthly"<?php if (esc_attr(strtolower(get_option('updates2slack_frequency'))) == 'monthly') {
		      echo ' selected';
		  } ?>>Monthly</option>
	   </select>
  </td>
    ...
    if (isset($_GET) && isset($_GET['testbutton']) && sanitize_text_field($_GET['testbutton']) == 1 && ! $pageWasRefreshed) {
        ?>
				      ...
				      <?php } ?>
					  <a class="button" <?php if (strlen(trim(esc_textarea(get_option('updates2slack_slackurls')))) == 0) {
					      echo 'href="javascript:void(0);" disabled';
					  } else {
					      echo ' href="'.$page_link.'&testbutton=1"';
					  } ?>>Trigger Slack Alert Now</a>
...
if (! function_exists('updates2slack_callback')) {
    function updates2slack_callback()
    {
        ...
        update_option('updates2slack_enabled',       updates2slack_sanitize_select_option(sanitize_text_field($_POST['updates2slack_enabled']), ['no', 'yes']));
        ...
        $cron_frequency = updates2slack_sanitize_select_option(sanitize_text_field($_POST['updates2slack_frequency']), ['daily', 'weekly', 'monthly']);
...
function updates2slack_sanitize_select_option($posted_value, $possible_values = [])
{
